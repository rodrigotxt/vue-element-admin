<template>
  <div>
    ...
    <el-main>
      ...
      <el-form v-if="tipo == 'especies'" ref="form" :model="form" label-width="180px">
        <el-form-item label="Nome da Espécie">
          <el-col :span="6">
            <el-input v-model="form.name" required="true" @keyup.enter.native="saveDados" />
          </el-col>
        </el-form-item>
      ...
      <el-form v-if="tipo == 'racas'" ref="form" :model="form" label-width="180px">
        <el-form-item label="Nome da Raça">
          <el-col :span="6">
            <el-input v-model="form.name" required="true" @focus="getEspecies()" />
          </el-col>
        </el-form-item>
        <el-form-item v-if="config.especies" label="Espécie">
          <el-select v-model="form.especie" placeholder="selecione uma espécie">
            <el-option v-for="(especie, index) in config.especies" :key="index" :label="especie.name" :value="especie.id"/>
          </el-select>
        </el-form-item>
        <el-button size="large" type="success" @click="saveDados">Salvar</el-button>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    saveDados: function() {
      // return console.log(localForage)
      // return false;
      const self = this
      var id = this.id
      self.form.id = id
